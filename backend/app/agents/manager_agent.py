...
def run(reports: List[AgentReport]) -> ManagerReport:
    ...
    requested=[]
    for r in reports:
        if r.recommendation == "WATCHLIST" and r.score < 0.55:
            requested.append(f"Request targeted re-analysis from {r.agent_name}")
    if any(r.recommendation == "ACTION_REQUIRED" for r in reports):
        recommendation="ACTION_REQUIRED"
    elif disagreements or any(r.recommendation == "WATCHLIST" for r in reports):
    ...
    explanation = "Manager summarizes specialist outputs only. It does not alter specialist scores or book spend entries. Main reason codes: " + ", ".join(sorted(set(all_reasons)))
